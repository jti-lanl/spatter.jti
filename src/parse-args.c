...
void initialize_argtable()
{
    ...
    malloc_argtable[10] = kernelName      = arg_strn("k", "kernel-name", "<kernel>", 0, 1, "Specify the kernel you want to run. [Default: Gather, Options: Gather, Gather_Parts, Scatter, GS, MultiGather, MultiScatter]");
...
void parse_json_kernel(json_object_entry cur, char** argv, int i)
{
    if (!strcasecmp(cur.value->u.string.ptr, "SCATTER") ||
        ...
        !strcasecmp(cur.value->u.string.ptr, "GATHER_PARTS") ||
...
struct run_config *parse_runs(int argc, char **argv)
{
   ...
   if (kernelName->count > 0)
   {
        ...
        if (!strcasecmp("MULTISCATTER", kernel_name))
            ...
            rc->kernel=GATHER;
#ifdef USE_OPENMP
        else if (!strcasecmp("GATHER_PARTS", kernel_name))
            rc->kernel=GATHER_PARTS;
#endif
        else
        {
   ...
   if (delta_gather->count > 0)
    ...
    if (rc->kernel == SCATTER)
        ...
        sprintf(kernel_name, "%s%zu", "gather_parts", rc->vector_len);
